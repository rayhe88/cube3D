...
int gradientLines(int ncp, dataCritP *nnucCrit, dataCube cube,
  ...
  **********************************************************/
  ...
  for(i=0; i < attractors; i++){
    ...
    if( np == 230 ) Lebedev0230(rleb);
    if( np ==  86 ) Lebedev0086(rleb);
    ...
    printf(" Attractor [%3d] (% 10.6lf % 10.6lf % 10.6lf) = % 10.6lf Rad % 10.6lf\n",
...
int gradientLines2(int bcp, dataCritP *bondCrit, dataCube cube,
                  dataRun param, double min0,const double *matU,
                  char *name){
  int np;
  int i,k,iter;
...
int gradientLines3(int bcp, int rcp, int ccp, 
                   dataCritP *bondCrit, dataCritP *ringCrit,
                   dataCritP *cageCrit,dataCube cube,
  ...
  **********************************************************/
  ...
  for(i=0; i < rcp; i++){
    ...
    matH[3] = val[7]; matH[4] = val[5]; matH[2] = val[9];
    matH[6] = val[8]; matH[7] = val[9]; matH[2] = val[6];

    JacobiNxN(matH,eval,evec);
    // The 1st eivec is taken
    vec1[0] = evec[3]; vec1[1] = evec[4]; vec1[2] = evec[5];
    vec2[0] = evec[6]; vec2[1] = evec[7]; vec2[2] = evec[8];
    
    nv1 = getNormVec(vec1);
    nv2 = getNormVec(vec2);

    vec1[0] /= nv1; vec1[1] /= nv1; vec1[2] /= nv1;
    vec2[0] /= nv2; vec2[1] /= nv2; vec2[2] /= nv2;


    for ( k = 0; k < np; k++ ){
      theta = k * dtheta;
      ri[0] = rc[0] + dr * cos(theta) * vec1[0] + dr * sin(theta) * vec2[0];
      ri[1] = rc[1] + dr * cos(theta) * vec1[1] + dr * sin(theta) * vec2[1];
      ri[2] = rc[2] + dr * cos(theta) * vec1[2] + dr * sin(theta) * vec2[2];
...
int gradientLines4(int bcp, int rcp, int ccp, 
                   dataCritP *bondCrit, dataCritP *ringCrit,
                   dataCritP *cageCrit,dataCube cube,
                   dataRun param, double min0,
                   const double *matU, char *name){
  ...
  **********************************************************/
  ...
  for(i=0; i < rcp; i++){
    ...
    vec0[0] = evec[0]; vec0[1] = evec[1]; vec0[2] = evec[2];
    vec1[0] = evec[3]; vec1[1] = evec[4]; vec1[2] = evec[5];
    vec2[0] = evec[6]; vec2[1] = evec[7]; vec2[2] = evec[8];
    ...
    vec0[0] /= nv0; vec0[1] /= nv0; vec0[2] /= nv0;
    vec1[0] /= nv1; vec1[1] /= nv1; vec1[2] /= nv1;
    vec2[0] /= nv2; vec2[1] /= nv2; vec2[2] /= nv2;
/*
    for(k=0;k<2;k++){
      ...
      fprintf(tmp," V1%04d  % 10.6lf   % 10.6lf  % 10.6lf\n",i+1,rn[0]*B2A,rn[1]*B2A,rn[2]*B2A);
    }
    for(k=0;k<2;k++){
      rn[0] = rc[0] - k * 0.75 * vec0[0];
      rn[1] = rc[1] - k * 0.75 * vec0[1];
    ...
    for(k=0;k<2;k++){
      ...
      fprintf(tmp," V2%04d  % 10.6lf   % 10.6lf  % 10.6lf\n",i+1,rn[0]*B2A,rn[1]*B2A,rn[2]*B2A);
    }
    for(k=0;k<2;k++){
      rn[0] = rc[0] - k * 0.75 * vec1[0];
      rn[1] = rc[1] - k * 0.75 * vec1[1];
    ...
    for(k=0;k<2;k++){
      ...
      fprintf(tmp," V3%04d  % 10.6lf   % 10.6lf  % 10.6lf\n",i+1,rn[0]*B2A,rn[1]*B2A,rn[2]*B2A);
    }
    for(k=0;k<2;k++){
      rn[0] = rc[0] - k * 0.75 * vec2[0];
      rn[1] = rc[1] - k * 0.75 * vec2[1];
...
*/
    for( j=0; j<bcp; j++){
        ...
        if( distance(rc,rj) < 5.66918 ){  // 3 Angstrom = 5.66918 Bohr
            ...
            nv = getNormVec(vij);
            vij[0] /= nv;  vij[1] /= nv;  vij[2] /= nv;

            cop = dotProduct(vec0,vij);
            //printf(" Cop  %3d %3d distance = % 10.6lf = % 10.6lf = % 10.6lf\n",i,j,
            //distance(rc,rj),cop,acos(cop)*180./M_PI);
