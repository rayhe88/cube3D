...
void InitPlot(FILE *gp, char *name, char *background, int type, int npx,
              int npy, int key) {

    fprintf(
        gp,
        "set term qt size %d,%d enhanced font \'Helvetica,11\' title \"%s\"\n",
        npx, npy, name);
    fprintf(
        gp,
        "set object rectangle from screen 0,0 to screen 1,1 behind fillcolor "
        "rgb \"%s\" fillstyle solid noborder\n",
        background);
    ...
    fprintf(gp,
            "set style line 401 lt 1 pt 7 ps 0.25 lw 2 lc rgb \'#FFF59D\'\n");
...
void InitPlotWx(FILE *gp, char *name, char *background, int type, int npx,
                int npy, int key) {

    fprintf(
        gp,
        "set term wxt size %d,%d enhanced font \'Helvetica,11\' title \"%s\"\n",
        npx, npy, name);
    fprintf(
        gp,
        "set object rectangle from screen 0,0 to screen 1,1 behind fillcolor "
...
void plotNCIdat(char *name, dataRun param) {
    ...
    fprintf(gp,
            " set title  'Non-covalentet Interactions index' tc rgb 'white'\n");
    fprintf(gp,
            " set xlabel 'sign({/Symbol l}_2){/Symbol r}(r)' tc rgb 'white'\n");
...
void getNCIgnu(char *name, dataRun param) {
    ...
    fprintf(
        tmp,
        " set term post enhanced color eps size 15cm,10cm font 'Arial,30' \n");
