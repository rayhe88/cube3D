...
void transform(double *v, double *theta, double *out) {
    ...
    out[1] = cx * (sz * v[0] + cz * v[1]) -
             sx * (-sy * (cz * v[0] - sz * v[1]) + cy * v[2]);
    out[2] = sx * (sz * v[0] + cz * v[1]) +
             cx * (-sy * (cz * v[0] - sz * v[1]) + cy * v[2]);
}
...
void transformInv(double *v, double *theta, double *out) {
    ...
    out[0] = -sz * (cx * v[1] - sx * v[2]) +
             cz * (cy * v[0] + sy * (sx * v[1] + cx * v[2]));
    out[1] = cz * (cx * v[1] - sx * v[2]) +
             sz * (cy * v[0] + sy * (sx * v[1] + cx * v[2]));
