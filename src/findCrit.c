...
int critPoints(dataCube cube, dataRun param, dataRC config, const double *matU,
               double min0, char *name) {
...
#pragma omp parallel private(i, j, k, nu, mu, idx, x, y, z, rin, rout, xp, yp, \
                             zp, xpp, ypp, zpp, ret, xx, yy, zz, f, limits)    \
    shared(nc1, nc2, nct, ctrl, x0, y0, z0, hx, hy, hz, cube, matU, param,     \
           min0, tmp)
    {
#pragma omp single
        {
...
#pragma omp for schedule(dynamic)
        for (mu = 0; mu < nct; mu++) {
            ...
            if (ctrl[mu] == 0) {
                ...
                rin[1] = yp;
                rin[2] = zp;
                ret[0] = rejectCube(rin, min0, cube, matU, param, config,
                                    limits, xx, yy, zz, f, rout[0]);

                rin[0] = xp;
                rin[1] = yp;
                rin[2] = zpp;
                ret[1] = rejectCube(rin, min0, cube, matU, param, config,
                                    limits, xx, yy, zz, f, rout[1]);

                rin[0] = xp;
                rin[1] = ypp;
                rin[2] = zp;
                ret[2] = rejectCube(rin, min0, cube, matU, param, config,
                                    limits, xx, yy, zz, f, rout[2]);

                rin[0] = xp;
                rin[1] = ypp;
                rin[2] = zpp;
                ret[3] = rejectCube(rin, min0, cube, matU, param, config,
                                    limits, xx, yy, zz, f, rout[3]);

                rin[0] = xpp;
                rin[1] = yp;
                rin[2] = zp;
                ret[4] = rejectCube(rin, min0, cube, matU, param, config,
                                    limits, xx, yy, zz, f, rout[4]);

                rin[0] = xpp;
                rin[1] = yp;
                rin[2] = zpp;
                ret[5] = rejectCube(rin, min0, cube, matU, param, config,
                                    limits, xx, yy, zz, f, rout[5]);

                rin[0] = xpp;
                rin[1] = ypp;
                rin[2] = zp;
                ret[6] = rejectCube(rin, min0, cube, matU, param, config,
                                    limits, xx, yy, zz, f, rout[6]);

                rin[0] = xpp;
                rin[1] = ypp;
                rin[2] = zpp;
                ret[7] = rejectCube(rin, min0, cube, matU, param, config,
                                    limits, xx, yy, zz, f, rout[7]);
...
int rejectCube(double *rin, double min0, dataCube cube, const double *matU,
               dataRun param, dataRC config, double *limits, double *xx,
               double *yy, double *zz, double *f, double *rout) {
    ...
    gradient3DLog(xi, yi, zi, xx, yy, zz, f, param.pol, param.orth, matU, min0,
                  val);
    ...
    if (fun < config.crit_tolfun && norm > config.crit_tolnrm) {
        ...
        while ((iter < config.crit_maxiter1) && (norm > config.crit_tolgrd) &&
               (difcoor == 0)) {
...
int numCritical01Vec(double r[3], dataCube cube, dataRun param,
                     const double *matU, double min0, double *val) {
...
int numCritical01(double x, double y, double z, dataCube cube, dataRun param,
                  const double *matU, double min0, double *val) {
    ...
    loadLocalField(idx, cube, param, xx, yy, zz, f);
    gradient3DLog(x, y, z, xx, yy, zz, f, param.pol, param.orth, matU, min0,
                  val);
    // gradient3D(x,y,z,xx,yy,zz,f,param.pol,param.orth,matU,val);
...
int numCritical02Vec(double r[3], dataCube cube, dataRun param,
                     const double *matU, double min0, double *val) {
...
int numCritical02(double x, double y, double z, dataCube cube, dataRun param,
                  const double *matU, double min0, double *val) {
    ...
    loadLocalField(idx, cube, param, xx, yy, zz, f);
    getDerivatives3DLog(x, y, z, xx, yy, zz, f, param.pol, param.orth, matU,
                        min0, val);
    // getDerivatives3D(x,y,z,xx,yy,zz,f,param.pol,param.orth,matU,val);

    return 0;
}
int numCritical02Vec_exp(double r[3], dataCube cube, dataRun param,
                         const double *matU, double min0, double *val) {
    int i, idx[3];
    double f[param.size];
    double xx[param.pol + 1];
    ...
    getDerivatives3D(r[0], r[1], r[2], xx, yy, zz, f, param.pol, param.orth,
                     matU, val);

    return 0;
}

double getFunInCube(int i, int j, int k, int n1, int n2, double min0, double *h,
                    double *field, double *norm) {
...
int refineCrit(int npc, double min0, dataCube cube, dataRun param,
               dataRC config, double *coorIn, const double *matU, char *name) {
    ...
    for (mu = 0; mu < npc; mu++) {
        ...
        if (iter == config.crit_maxiter2) {
            while (iter < 2 * config.crit_maxiter2 &&
                   norm > config.crit_tolgrd) {
                x = (x + coorIn[3 * mu]) / 2.;
                y = (y + coorIn[3 * mu + 1]) / 2.;
                z = (z + coorIn[3 * mu + 2]) / 2.;
                numCritical02(x, y, z, cube, param, matU, min0, val);
...
int describeCrit(int ncrit, double min0, dataCube cube, dataRun param,
    ...
    if (rcp != 0)
        createArrayCritP(rcp, &ringCrit, "Critical RCP");
    if (ccp != 0)
        createArrayCritP(ccp, &cageCrit, "Critical CCP");
    if (ncp != 0)
        createArrayCritP(ncp, &nnucCrit, "Critical NCP");
    if (xcp != 0)
        createArrayCritP(xcp, &degeCrit, "Critical XCP");
    ...
    if (bcp != 0)
        sortCritPoints(bcp, bondCrit);
    if (rcp != 0)
        sortCritPoints(rcp, ringCrit);
    if (ccp != 0)
        sortCritPoints(ccp, cageCrit);
    if (ncp != 0)
        sortCritPoints(ncp, nnucCrit);
    if (xcp != 0)
        sortCritPoints(xcp, degeCrit);
    ...
    bondPath(bcp, bondCrit, ncp, nnucCrit, bonding, cells, cube, param, config,
             min0, matU, name);
