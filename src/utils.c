...
void printRunning(dataRun param) {
    ...
    case CRI:
        ...
        break;
    case VOI:
        strcpy(ascTask, "VOIDS");
        break;
    case REP:
        strcpy(ascTask, "REPLI");
        break;
    }
...
void printCubeRot(int rotate, const char *name, dataCube cube) {
    ...
    if (rotate == YES) {
        ...
        printCube("Cube in the format of fraccional coordinates Cube3D", cube,
                  out);
        fclose(out);
...
void printCube(char *text, dataCube c, FILE *out) {
    ...
    fprintf(out, " File generated by Cube3D-%s code\n", VERSION);
    fprintf(out, " %s\n", text);
    ...
    for (i = 0; i < c.natm; i++)
        fprintf(out, " %5d % 10.6lf % 10.6lf % 10.6lf % 10.6lf\n", c.zatm[i],
                (double)c.zatm[i], c.coor[3 * i], c.coor[3 * i + 1],
                c.coor[3 * i + 2]);
...
void printDataNCI(int npt, dataRun param, double *fred, double *frho,
                  char *name) {
    ...
    fprintf(out, "########################################################\n");
    fprintf(out, "#   File 100*lambda_2*rho(r) vs s(r) create by Cube3D-%s\n",
            VERSION);
...
void checkCommandLine(int argc, char *argv[]) {
    ...
    if (flags[0] == 1) {
        printf("%s A input file [%s] will be created!%s\n", TGBI, nameInput,
               TRST);
        if (flags[1] == 0)
            printf("%s Change the name 'foobar.cube' in the file [%s]%s\n",
                   TRBI, nameInput, TRST);

        createInput(nameCube, nameInput);
...
void createInput(char *nameCube, char *nameInp) {
    ...
    fprintf(
        fin,
        "# 2 and 3 are suitable for critical points, 4 takes a long time\n");
    fprintf(fin, ">> POLY\n");
    fprintf(fin, "3\n");
    ...
    fprintf(fin, "#1 2 3\n");
    fprintf(fin,
            "# If this line is added it is optional the field is calculated\n");
    ...
    fprintf(fin, "# The following line has been added to give more control to "
                 "GEOM PLANE,\n");
    fprintf(fin, "# it only works when the latter flag is activated.\n");
    fprintf(fin,
            "# FIELD   Calculates the field given by TASK( REDU, GRAD, LAP, "
            "KIN, VIR, KEDW).\n");
    fprintf(fin, "#         A gnuplot file with name *planeAt1-At2-At3.dat is "
                 "generated with\n");
    fprintf(fin, "#         information of the coordinates, the TASK field and "
                 "the value of the\n");
    fprintf(fin, "#         original field at that point.\n");
    fprintf(fin,
            "# GLINE   Determines the streamlines, in the 2D plane, of the "
            "vector field\n");
    fprintf(fin,
            "#         formed by the scalar field read from the cube file. "
            "This option\n");
    fprintf(fin, "#         generates 2 files gnuplot.\n");
    fprintf(fin, "#         The streamlines are in the "
                 "*planeAt1-At2-At3-gLine.dat file, while\n");
    fprintf(fin, "#         the stream direction is given like vectors in the "
                 "file *-gLineVec.dat.\n");
    fprintf(fin, "# GVECTOR Calculates the vector field in the 2D plane, from "
                 "the scalar field\n");
