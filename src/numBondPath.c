...
#pragma omp parallel private(i, nucleo1, nucleo2, val, matH, eval, evec, vec,  \
                             vec2, norm, ri, rn, qi, qn, iterp, itern, dist,   \
...
#pragma omp for schedule(dynamic)
        for (i = 0; i < bcp; i++) {
            ...
            fprintf(tmp, "%d % 10.6lf % 10.6lf % 10.6lf % 10.6lf\n", i + 1,
                    rc[0] * B2A, rc[1] * B2A, rc[2] * B2A, dij);
...
int logFile(int bcp, int rcp, int ccp, int ncp, dataCritP *bondCrit,
    ...
    if (ncp > 0) {
        ...
        for (i = 0; i < ncp; i++) {
            ...
            fprintf(out,
                    "   Coordinates [A]    % 12.8E  % 12.8E  % 12.8E  \n\n",
                    r[0], r[1], r[2]);
...
void getEnergies(double rho, double lap, double *kinG, double *kinK,
                 double *vir) {
...
void printLog1(int nbc, int nrc, int ncc, int nna, int nnu, FILE *out) {
    ...
    fprintf(out, " This file contains information about critical points and "
                 "properties, these\n");
    fprintf(out, " information are determineted by code Cube3D-%s\n", VERSION);
    fprintf(out,
            " The units in this  file for  distance are  Angstrom and  the "
            "units for fields are\n");
    ...
    fprintf(out, "  Kinetic Energy G    Kinetic Energy Density in the form "
                 "Lagrangian\n");
    fprintf(out, "                      (Abramov's approximation)\n");
    fprintf(out, "  Kinetic Energy K    Kinetic Energy Density in the form "
                 "Hamiltonian\n");
    fprintf(out,
            "  Virial Field        Or Potential Energy Density  V = -K - G\n");
    // fprintf(out,"  Lagrangian Density  -(1/4) Laplacian or K - G\n");
    // fprintf(out,"  KenergyG/Density    Kinetic Energy Density per
    // electron\n");

    fprintf(out, "  Eigenvalues         Eigenvalues for Hessian matrix: "
    ...
    printBar82(out);
    fprintf(out,
            "  Atractor                Coordinates [A]            Density   "
            "      Laplacian\n");
    printBar82(out);
}
...
int logFileCSV(int ncp, dataCritP *crit, dataCube cube, dataRun param,
    ...
    fprintf(out,
            "lambda_1, lambda_2, lambda_3, ellepticiy, energyInt(kcal/mol)\n");
    ...
    for (i = 0; i < ncp; i++) {
        ...
        r[1] *= B2A;
        r[2] *= B2A;
        fprintf(out, "%3d,%s,%10.6lf,%10.6lf,%10.6lf,", i + 1, string, r[0],
                r[1], r[2]);
        fprintf(out, "%10.6lf,%10.6lf,%10.6lf,%10.6lf,%10.6lf,", fun, ngrad,
                lap, kinG, kinK);
