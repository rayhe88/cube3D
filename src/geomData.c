...
/**********************************************************************************/
  for( i = 0; i < cube.natm; i++ ){
      rin[0] = cube.coor[3*i];  rin[1] = cube.coor[3*i+1];  rin[2] = cube.coor[3*i+2];

      trans00(rin,matU);

      cube.coor[3*i] = rin[0];  cube.coor[3*i+1] = rin[1];  cube.coor[3*i+2] = rin[2];
  }
  rin[0] = cube.min[0];  rin[1] = cube.min[1];  rin[2] = cube.min[2];
  trans00(rin,matU);
  cube.min[0] = rin[0];  cube.min[1] = rin[1];  cube.min[2] = rin[2];

  rin[0] = cube.max[0];  rin[1] = cube.max[1];  rin[2] = cube.max[2];
  trans00(rin,matU);
  cube.max[0] = rin[0];  cube.max[1] = rin[1];  cube.max[2] = rin[2];
/**********************************************************************************/
...
/**********************************************************************************/
  ...
  printf("   Centre %3d:  % 10.6lf % 10.6lf % 10.6lf [A]\n",at1+1,r1[0]*B2A,r1[1]*B2A,r1[2]*B2A);
  printf("   Centre %3d:  % 10.6lf % 10.6lf % 10.6lf [A]\n",at2+1,r2[0]*B2A,r2[1]*B2A,r2[2]*B2A);
  printf("   Centre %3d:  % 10.6lf % 10.6lf % 10.6lf [A]\n",at3+1,r3[0]*B2A,r3[1]*B2A,r3[2]*B2A);

  dist = getVecLMN(r1,r2,r3,r0,rcg,vL,vM,vN);
  n = ceil(dist*(NPUA1));
  h = dist/(double) (n-1);

  printf("   Baricentre:  % 10.6lf % 10.6lf % 10.6lf [A]\n",rcg[0]*B2A,rcg[1]*B2A,rcg[2]*B2A);
  ...
  fprintf(out,"#  Baricentre   (% 7.3lf % 7.3lf % 7.3lf) -> (% 7.3lf, % 7.3lf) [A]\n",
               rcg[0]*B2A,rcg[1]*B2A,rcg[2]*B2A,ejeM*B2A,ejeL*B2A);

  trans3Dto2D(r0, vL, vM, cgL, cgM, &ejeL, &ejeM);
  fprintf(out,"#  Origin Plane (% 7.3lf % 7.3lf % 7.3lf) -> (% 7.3lf, % 7.3lf) [A]\n",
...
// New feature1
int getGradLinesInPlane1(double min0,dataCube cube, dataRun param, const double *matU,char *name){
...
/**********************************************************************************/
  ...
  ri[0][0] = r1[0];  ri[0][1] = r1[1];  ri[0][2] = r1[2];
  ri[1][0] = r2[0];  ri[1][1] = r2[1];  ri[1][2] = r2[2];
  ri[2][0] = r3[0];  ri[2][1] = r3[1];  ri[2][2] = r3[2];
  ...
  n = ceil(dist*(NPUA2));
  h = dist/(double) (n-1);
  ri[3][0] = rcg[0];  ri[3][1] = rcg[1];  ri[3][2] = rcg[2];
  ...
  d2min[0] =  ejeL; d2min[1] =  ejeM;
  d2max[0] = -ejeL; d2max[1] = -ejeM;
  ...
  for(i=0; i < 3; i++){
    ...
    cL = ejeL * B2A;
    for( j = 0; j < NANG; j++){
        rj[0] =  eps * vL[0] * sin(j * theta) + eps * vM[0] * cos(j * theta) + ri[i][0];
        rj[1] =  eps * vL[1] * sin(j * theta) + eps * vM[1] * cos(j * theta) + ri[i][1];
        rj[2] =  eps * vL[2] * sin(j * theta) + eps * vM[2] * cos(j * theta) + ri[i][2];
        ...
        while( flag == 0 && iter < 10000){
            ...
            loadLocalField(idx,cube,param,xx,yy,zz,f);
            gNum(qi[0],qi[1],qi[2],xx,yy,zz,f,param.pol,param.orth,matU,min0,val);
...
// New feature2
int getGradVectorsInPlane(double min0,dataCube cube, dataRun param, const double *matU,char *name){
...
/**********************************************************************************/
  ...
  for(i=0;i<n;i++){
    for(j=0;j<n;j++){
        ...
        getIndex3D(cube.pts,q,cube.min,cube.hvec,idx);
        loadLocalField(idx,cube,param,xx,yy,zz,f);
        gradient3DLog(q[0],q[1],q[2],xx,yy,zz,f,param.pol,param.orth,matU,min0,val);
        f0 = getGrd(val);
        ...
        fprintf(out," % 12.6lf % 12.6lf % 12.6lf % 12.6lf  % 14.8lf % 14.8lf\n",ejeM,ejeL,gM, gL, f0,val[0]);
...
void printInfoCube1( dataCube cube){
    ...
    for(i=0; i<cube.natm; i++)
        printf(" Atm%2d :  % 12.6lf  % 12.6lf  % 12.6lf\n",i,
                           cube.coor[3*i], cube.coor[3*i+1], cube.coor[3*i+2]);

}

int getStreamLinesInPlane(double min0,dataCube cube, dataRun param, const double *matU,char *name){
...
/**********************************************************************************/
  ...
  fprintf(vec,"#      coor l       coor m       vec l        vec m        fun ");

  double min2[2], max2[2];
  min2[0] = ejeL; min2[1] = ejeM;
  max2[0] = -ejeL; max2[1] = -ejeM;
  ...
  basisLMN[0][0] = vL[0]; basisLMN[0][1] = vL[1]; basisLMN[0][2] = vL[2];
  basisLMN[1][0] = vM[0]; basisLMN[1][1] = vM[1]; basisLMN[1][2] = vM[2];
  basisLMN[2][0] = vN[0]; basisLMN[2][1] = vN[1]; basisLMN[2][2] = vN[2];
  basisLMN[3][0] = cgL;   basisLMN[3][1] = cgM;   basisLMN[3][2] = cgN;
  ...
  getStreamLine( id, n, nline, min0, cube, param, matU, matT, basisLMN, min2, max2, matrix, out, vec);
...
double EulerMethod(double rin[3],double rout[3],dataCube cube, dataRun param, const double *matU, const double *matT, double min0, int sign){
...
double RungeKutta4_fast(double rin[3],double rout[3],dataCube cube, dataRun param, const double *matU, const double *matT, double min0, int sign){
...
double RungeKutta4(double rin[3],double rout[3],dataCube cube, dataRun param, const double *matU, const double *matT, double min0, int sign){
...
int getStreamLine( int id0, int n, int nline, double min0,dataCube cube,dataRun param,
                   const double *matU, const double *matT, double basisLMN[4][3],
                   double min[2], double max[2], dataSLine *matrix, FILE *out, FILE *vec){
    ...
    trans3Dto2D(ri, basisLMN[0], basisLMN[1], basisLMN[3][0], basisLMN[3][1], &coorL, &coorM);
    ...
    while( goon == 0 && iter < MAXITER){
        ...
        trans3Dto2D(rout, basisLMN[0], basisLMN[1], basisLMN[3][0], basisLMN[3][1], &coorL, &coorM);
...
// Check the point given by cooL and coorM is inside the region  [min:max][min:max]
// and the value of the gradient norm will be greater than 1.E-7
int check( double coorL, double coorM, double min[2], double max[2], double val[10]){
    int ret = 0;
...
int checkMatrix(int n, dataSLine *matrix, double coorL, double coorM, int id0){
    ...
    for(id=0; id < n*n; id++){
        dist = sqrt( pow(coorL - matrix[id].s[0],2) + pow(coorM - matrix[id].s[1],2));
        if( dist < distmin){
            idmin = id;
            distmin = dist;
        }
...
void trans3Dto2D(double ri[3], double vL[3], double vM[3],
                 double cgL, double cgM, double *coorL, double *coorM){
...
void trans2Dto3D(double coorL, double coorM,double basisLMN[4][3],double rout[3]){

    double vL[3], vM[3], vN[3], cg[3];

    vL[0] = basisLMN[0][0]; vL[1] = basisLMN[0][1]; vL[2] = basisLMN[0][2];
    vM[0] = basisLMN[1][0]; vM[1] = basisLMN[1][1]; vM[2] = basisLMN[1][2];
    vN[0] = basisLMN[2][0]; vN[1] = basisLMN[2][1]; vN[2] = basisLMN[2][2];
    cg[0] = basisLMN[3][0]; cg[1] = basisLMN[3][1]; cg[2] = basisLMN[3][2];
